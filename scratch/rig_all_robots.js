...
function rigRobotSvg(filePath, options = {}) {
    const content = fs.readFileSync(filePath, 'utf8');
    const match = content.match(/<image[^>]+xlink:href="([^"]+)"/);
    if (!match) {
        console.error('Could not find base64 image in', filePath);
        return;
    }
    const base64 = match[1];

    // Build the 672x1024 hierarchical SVG
    const riggedSvg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 672 1024" width="100%" height="100%" id="buddy-root">
  <defs>
    <!-- Master High-Resolution Character Asset Source -->
    <image id="buddy-char-asset" width="672" height="1024" preserveAspectRatio="none" xlink:href="${base64}"/>

    <!-- Accurate Anatomical Clip Paths for Body Parts -->
    <!-- Head & Visor & Crown / Antenna Clip -->
    <clipPath id="clip-buddy-head">
      <path d="M 336 2 C 190 2 90 150 90 320 C 90 435 190 472 336 472 C 482 472 582 435 582 320 C 582 150 482 2 336 2 Z"/>
    </clipPath>
    ...
    <!-- Torso & Pelvis Clip (including chest core) -->
    <clipPath id="clip-buddy-body">
      <path d="M 210 465 L 462 465 C 495 510 495 630 460 678 L 212 678 C 177 630 177 510 210 465 Z"/>
    </clipPath>

    <!-- Left Arm Assembly Clips (Shoulder -> Forearm -> Hand) -->
    <clipPath id="clip-buddy-left-upper-arm">
      <path d="M 170 445 C 235 445 250 550 205 605 C 160 635 120 595 125 540 C 125 500 140 445 170 445 Z"/>
    </clipPath>
    <clipPath id="clip-buddy-left-forearm">
      <path d="M 120 585 C 195 590 195 680 165 725 C 115 725 85 680 100 620 C 105 595 115 585 120 585 Z"/>
    </clipPath>
    <clipPath id="clip-buddy-left-hand">
      <path d="M 90 700 C 175 700 185 785 155 845 C 105 845 75 785 80 735 Z"/>
    </clipPath>

    <!-- Right Arm Assembly Clips (Shoulder -> Forearm -> Hand) -->
    <clipPath id="clip-buddy-right-upper-arm">
      <path d="M 502 445 C 437 445 422 550 467 605 C 512 635 552 595 547 540 C 547 500 532 445 502 445 Z"/>
    </clipPath>
    <clipPath id="clip-buddy-right-forearm">
      <path d="M 552 585 C 477 590 477 680 507 725 C 557 725 587 680 572 620 C 567 595 557 585 552 585 Z"/>
    </clipPath>
    <clipPath id="clip-buddy-right-hand">
      <path d="M 582 700 C 497 700 487 785 517 845 C 567 845 597 785 592 735 Z"/>
    </clipPath>

    <!-- Left Leg Assembly Clips (Hip -> Thigh -> Foot/Boot) -->
    <clipPath id="clip-buddy-left-thigh">
      <path d="M 205 660 L 315 660 L 315 780 L 205 780 Z"/>
    </clipPath>
    <clipPath id="clip-buddy-left-foot">
      <path d="M 155 765 L 325 765 L 325 960 L 155 960 Z"/>
    </clipPath>

    <!-- Right Leg Assembly Clips (Hip -> Thigh -> Foot/Boot) -->
    <clipPath id="clip-buddy-right-thigh">
      <path d="M 357 660 L 467 660 L 467 780 L 357 780 Z"/>
    </clipPath>
    <clipPath id="clip-buddy-right-foot">
      <path d="M 347 765 L 517 765 L 517 960 L 347 960 Z"/>
    </clipPath>
  </defs>
...
`;
    ...
    fs.writeFileSync(filePath, riggedSvg, 'utf8');
    console.log('Successfully rigged:', filePath);
}
...
allRobots.forEach(f => rigRobotSvg(f));
